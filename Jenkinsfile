    ...
    stage ('Push image into ECR') {
      steps {
        withEnv ([ "AWS_ACCESS_KEY_ID=${env.AWS_ACCESS_KEY_ID}", "AWS_SECRET_ACCESS_KEY=${env.AWS_SECRET_ACCESS_KEY}", "AWS_DEFAULT_REGION=${env.AWS_DEFAULT_REGION}" ]) {
          ...
          sh 'docker tag jenkins-image:latest 161146977543.dkr.ecr.ap-south-1.amazonaws.com/jenkins-image:latest'
          sh 'docker push 161146977543.dkr.ecr.ap-south-1.amazonaws.com/jenkins-image:latest'
        }
      }
    }
  }
